...
- **Branch Naming Convention (MANDATORY)**: 新規ブランチを作成する際は、`[type]/[ID]` 形式（例: `feat/v2.10.0`, `fix/v2.9.1`）を遵守せよ。この命名行為が、後続のIssueやシステム全体における `type` の決定（物理的証拠）となる。これ以外の命名は同期を破壊するため禁止する。
