...
## 14. Compressed Instruction (圧縮命令) へ置換する

長大な記述や重複する制約は、LLMが既知である「高密度な単語」で積極的に置換し、指示の情報を圧縮せよ。

- LLMの学習済み知識（Pre-trained knowledge）を活用し、一語で複数の制約をパッケージ化する。
- 冗長な説明を削り、意味の密度（Signal-to-noise ratio）を最大化する。

例：
- 「1つの操作で1つの変更のみを行い、他への影響を最小化する」 → **Atomic**
- 「入力に対し一意の出力を返し、推測や揺れを排除する」 → **Deterministic**
- 「社交辞令や冗長な説明を一切排し、情報の真偽のみを伝える」 → **High-signal**

**禁止**: 用語自体を抽象的な「目標」として記載すること。説明を省くために用語を使うのであり、両方を記述するのは情報の重複（Low-signal）である。

- × 「1つの操作で1つの変更のみを行い、他への影響を最小化するように **Atomic** に実装せよ」
- ○ 「**Atomic** に実装せよ」
...
- フォールバックが定義されている
- 思想ではなく拘束条件で記述されている
- **Compressed Instruction (圧縮命令) により情報密度が最大化されている**
- **High-signal（高情報密度）であり、解釈の揺れが最小化されている**
