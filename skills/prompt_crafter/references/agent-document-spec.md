...
## 14. Compressed Instruction (圧縮命令) を活用する

単なる単語ではなく、**特定の文脈、制約、動作原理を一語にパッケージ化し、解釈の余地を排除した「高密度な命令語」**を採用せよ。これにより、トークン消費を抑えつつ、エージェントの挙動を強力に拘束できる。

推奨される圧縮命令の例：

| 命令語 | 圧縮された文脈・制約 |
|--------|----------------------|
| **Atomic** | 「1つの操作で1つの変更のみを行う」「不可分である」「依存関係を最小化する」 |
| **Idempotent** | 「何回実行しても同じ結果になる」「副作用を制御する」「状態を再現可能にする」 |
| **High-signal** | 「意味のない冗長な表現を排除する」「情報密度を最大化する」「ノイズ（社交辞令等）を0にする」 |
| **Deterministic** | 「入力に対して出力が一意に決まる」「推測や揺れを排除する」「実行の再現性を担保する」 |

これらは「例」に過ぎない。重要なのは、設計者が「この一語でどのような分厚い制約をパッケージ化したか」を明確に意識し、それをエージェントとの共通言語として行使することである。
...
- フォールバックが定義されている
- 思想ではなく拘束条件で記述されている
- **Compressed Instruction (圧縮命令) により高密度に設計されている**
- **High-signal であり、解釈の揺れが最小化されている**
