...
- ?? **Experimental Agent Skills Support in Preview:** Gemini CLI now supports
  ...
  early preview where weÅfre looking for feedback!
...
- **?????? Gemini 3 Flash + Gemini CLI:** Better, faster and cheaper than 2.5
...
- **Zed integration:** Users can now leverage Gemini 3 within the Zed
  integration after enabling "Preview Features" in their CLIÅfs `/settings`.
...
- **Gemini 3 support for paid:** Gemini 3 support has been rolled out to all API
  ...
  **Preview Features**.
- **Updated UI rollback:** WeÅfve temporarily rolled back our updated UI to give
  it more time to bake. This means for a time you wonÅft have embedded scrolling
...
- **?? Seamless scrollable UI and mouse support:** WeÅfve given Gemini CLI a
...
- **Removal of deprecated flags:** WeÅfve finally removed a number of deprecated
  flags to cleanup Gemini CLIÅfs invocation profile:
...
- **Install pre-release extensions:** Install the latest `--pre-release`
  versions of extensions. Used for when an extensionÅfs release hasnÅft been
  marked as "latest".
...
- ?? **Higher limits for Google AI Pro and Ultra subscribers:** WeÅfre psyched to
...
- **Direct to Google Cloud Telemetry:** Directly send telemetry to Google Cloud
  ...
  [@jerop](https://github.com/jerop))
- **Visual Mode Indicator Revamp:** ÅeshellÅf, 'accept edits' and 'yolo' modes now
...
- **Edit Tool improvements:** Gemini CLIÅfs ability to edit files should now be
  far more capable. ([pr](https://github.com/google-gemini/gemini-cli/pull/7679)
  by [@silviojr](https://github.com/silviojr))
- **Custom witty messages:** The feature youÅfve all been waiting forÅc
...
- **Enforced authentication:** System administrators can now mandate a specific
  ...
  `"enforcedAuthType": "oauth-personal|gemini-api-key|Åc"`in `settings.json`.
...
- **Experimental**
  - **Edit Tool:** Give our new edit tool a try by setting
    ...
    [@silviojr](https://github.com/silviojr))
  - **Model talking to itself fix:** WeÅfve removed a model workaround that would
...
- **2.5 Flash Lite support:** You can now use the `gemini-2.5-flash-lite` model
  for Gemini CLI via `gemini -m Åc`.
