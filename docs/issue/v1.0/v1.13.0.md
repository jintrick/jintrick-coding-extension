...
### 4. テスト仕様 (Test Specifications)
以下のシナリオを `tests/hooks/command_fixer_hook.test.js` にて検証する。
- **正常系 (置換あり)**: `git add . && git commit` -> `git add . ; git commit`
- **正常系 (置換なし)**: `git status` (変更なし)
- **境界値 (スペースなし)**: `echo foo&&bar` (置換しない：文字列の一部の可能性があるため)
- **フィルタリング (ツール名)**: `write_file` 実行時 (介入しない)
- **フィルタリング (イベント)**: `AfterTool` イベント時 (介入しない)
- **異常系 (不正なJSON)**: stdin が JSON でない場合 (エラーログ出力の上で allow)

## ゴール
- [x] `run_shell_command` 実行時に `git add . && git commit` が `git add . ; git commit` として実行されること。
- [x] `hooks/hooks.json` に正しく登録されていること。
- [x] `npm run build` により `dist/hooks/command_fixer_hook.cjs` が生成されること。
- [x] 自動テスト（モック入力）により置換ロジックの正当性が検証されていること。
