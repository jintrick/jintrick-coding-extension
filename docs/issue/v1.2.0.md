...
- **実行判定ロジック**:
  - フック自体は `activate_skill` イベントで起動されるが、以下の条件をすべて満たす場合のみ更新処理を実行する。
    ...
    2. `docs/reference/gemini-cli-expert/references/discovery.json` の `metadata.updated_at` を確認し、前回の更新から 24 時間以上経過していること。
- **ドキュメント取得**:
  - `git clone --depth 1 https://github.com/google-gemini/gemini-cli.git` を一時ディレクトリ（`os.tmpdir()`）で実行する。
  - `gemini-cli/docs/` ディレクトリ配下の `.md` ファイルを、本プロジェクトの `docs/reference/gemini-cli-expert/references/docs/` にコピーする。
- **`discovery.json` の再生成**:
  - 同期した Markdown ファイルを再帰的にスキャンする。
  - 各ファイルの 1 行目（`# Title`）を `title` として取得。
  - 最初の段落（空行で区切られた最初のテキストブロック）を `summary` として取得。
  - `url` は `https://github.com/google-gemini/gemini-cli/blob/main/docs/` + 相対パスで生成。
  - **`metadata.updated_at` に現在のタイムスタンプ（ISO 8601）を記録**して、`docs/reference/gemini-cli-expert/references/discovery.json` に上書き保存する。
