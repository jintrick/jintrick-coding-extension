...
### Issue タイプの定義とバージョニング
Issue の `type` は、ツールチェーンによって自動推論される次期バージョンに影響する。

| タイプ | バージョニング | 定義・用途 |
| :--- | :--- | :--- |
| **feat** | Minor-up (vX.Y.0) | ユーザーに提供される新しい機能の追加。 |
| **refactor** | Minor-up (vX.Y.0) | コードの品質向上のための内部構造の変更。 |
| **fix** | Patch-up (vX.Y.Z+1) | バグや不具合の修正。 |
| **docs** | Patch-up (vX.Y.Z+1) | ドキュメントの追加・修正。 |
| **chore** | Patch-up (vX.Y.Z+1) | **基盤整備（将来の機能のための土壌作り）**、ビルドプロセス、依存ライブラリの更新など。 |

**注意**: 将来の機能追加のための「型定義のみ」や「インターフェースの枠組み」などの実装準備は、`feat` ではなく `chore` として起草すること。これにより、実機能が提供される前の段階での不必要なマイナーバージョン上昇（バージョンインフレ）を防止する。
