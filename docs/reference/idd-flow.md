...
3.  **自動同期**: `git commit -m "vX.Y.Z ..."` の実行時に、`IDD Sync Hook` が自動的にマニフェスト（`package.json` 等）のバージョンを更新・ステージングする。
...
2.  **バージョン同期とコミット**: `git commit -m "vX.Y.Z ..."` を実行する。これにより `IDD Sync Hook` が起動し、マニフェストファイルのバージョンを自動更新・追加ステージングした上でコミットが完了する。
