...
model: gemini-3-flash-preview  # 省略時は 'inherit'
temperature: 0.7
max_turns: 15
timeout_mins: 5
kind: local
...
- **`name`** (string): 必須。一意の識別子。ユーザーが `@name` で呼び出す際の名称となる（小文字、数字、ハイフン、アンダースコアのみ）。
- **`description`** (string): 必須。エージェントが何を行うかの短い説明。メインエージェントが委譲を判断する材料となる。
- **`model`** (string): 任意。使用する特定のAIモデル（例: `gemini-3-flash-preview`）。省略時または `inherit` を指定した場合は、メインセッションのモデルを継承する。
- **`temperature`** (number): 任意。モデルの生成温度（0.0 - 2.0）。
- **`max_turns`** (number): 任意。このエージェントが完了前に許可される最大会話ターン数。デフォルトは `15`。
- **`timeout_mins`** (number): 任意。最大実行時間（分）。デフォルトは `5`。
- **`kind`** (string): 任意。実行環境。`local`（デフォルト）または `remote`。
- **`tools`** (array): 任意（重要）。エージェントの使用を許可するツールのリスト。省略した場合はデフォルトのツールセットにアクセス可能となる。
    - **制約**: サブエージェントが使用できないツール（例: 他の高度なサブエージェント等）をリストに含めると、**ロードに失敗し CLI に認識されなくなる**ため、明示的に指定する場合は最小限かつ確実なツールのみとすること。
